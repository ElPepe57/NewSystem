    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      >
        {reservasPorVencer.length === 0 ? (
          <p className="text-sm text-slate-500 py-2">No hay reservas próximas a vencer</p>
        ) : (
            ...
            {reservasPorVencer.map(r => (
              <div key={r.id} className="flex items-center justify-between p-2 bg-red-50 rounded">
                <div>
                  ...
                  <span className="text-xs text-slate-500 ml-2">{r.nombreCliente}</span>
                </div>
      ...
      >
        {conFaltanteStock.length === 0 ? (
          <p className="text-sm text-slate-500 py-2">Todas las cotizaciones tienen stock</p>
        ) : (
          <div className="space-y-2 pt-2">
            {conFaltanteStock.slice(0, 5).map(c => (
              <div key={c.id} className="flex items-center justify-between p-2 bg-amber-50 rounded">
                <div>
                  ...
                  <span className="text-xs text-slate-500 ml-2">{c.nombreCliente}</span>
                </div>
            ...
            {conFaltanteStock.length > 5 && (
              <p className="text-xs text-slate-500 text-center">+{conFaltanteStock.length - 5} más</p>
            )}
          </div>
        )}
      </SeccionColapsable>
      ...
      >
        {reservasVirtuales.length === 0 ? (
          <p className="text-sm text-slate-500 py-2">No hay reservas virtuales pendientes</p>
        ) : (
          <div className="space-y-2 pt-2">

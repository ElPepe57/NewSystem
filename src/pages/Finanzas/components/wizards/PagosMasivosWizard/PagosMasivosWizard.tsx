...
import { DocumentosPendientesTable } from '../pagosMasivos/DocumentosPendientesTable';
import { ProgresoEjecucion } from '../pagosMasivos/ProgresoEjecucion';
...
import { type WizardStepKey } from './StepsSidebar';
import { ResumenSidebar } from './ResumenSidebar';
import {
  WizardShellSidebar,
  type WizardPasoItem,
  type WizardContextoItem,
} from '../shells/WizardShellSidebar';
 ...
 * (sprint S7.SF2). El wizard ahora se invoca SÓLO como modal desde
 * el shell Finanzas (FinanzasLayout dropdown "+ Nuevo movimiento").
 */
export interface PagosMasivosWizardProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export const PagosMasivosWizard: React.FC<PagosMasivosWizardProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  // ─── Build pasos para WizardShellSidebar canon ────────────────────
  const pasosShell: WizardPasoItem[] = [
    { numero: 1, label: 'Tipo + Cuenta', completado: pasosCompletados.has('tipo') && paso !== 'tipo', actual: paso === 'tipo' },
    { numero: 2, label: 'Documentos', completado: pasosCompletados.has('documentos') && paso !== 'documentos', actual: paso === 'documentos' },
    { numero: 3, label: 'Revisión', completado: pasosCompletados.has('revision') && paso !== 'revision', actual: paso === 'revision' },
    { numero: 4, label: 'Ejecución', completado: false, actual: paso === 'ejecucion' },
  ];

  const contextoShell: WizardContextoItem[] = [
    { label: 'Tipo lote', valor: tipoLote === 'egreso' ? 'Egreso (pagos)' : 'Ingreso (cobros)' },
    { label: 'Items', valor: `${itemsArray.length} seleccionados` },
  ];
  ...
  // ─── chk5.D-S7.SF5 · Render modal canon · único modo soportado ────
  const pasoLabel: Record<WizardStepKey, string> = {
    tipo: 'Tipo + Cuenta',
  ...
  return (
    <WizardShellSidebar
      isOpen={isOpen}
      onClose={onClose}
      onAtras={paso !== 'tipo' ? handleRetroceder : undefined}
      onSiguiente={handleAvanzar}
      onSubmit={handleAvanzar}
      tono="indigo"
      titulo="Pagos masivos · batch"
      pasos={pasosShell}
      contexto={contextoShell}
      topBarLabel={`PASO ${pasosShell.find((p) => p.actual)?.numero} · ${pasoLabel[paso]}`}
      topBarSubtitulo={
        tipoLote === 'egreso' ? 'Lote de egresos (pagos)' : 'Lote de ingresos (cobros)'
      }
      siguienteLabel={paso === 'revision' ? 'Ejecutar lote' : 'Siguiente'}
      siguienteEsSubmit={paso === 'revision'}

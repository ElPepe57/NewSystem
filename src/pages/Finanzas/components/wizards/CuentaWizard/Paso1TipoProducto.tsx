...
const TIPO_OPTIONS: Array<{
  ...
  // chk5.D-S1f · F4 · 6to tipo Caja Recaudadora (D5 + D12)
  {
    value: 'recaudadora',
    label: 'Recaudadora',
    icon: Truck,
    hint: 'Tercero (ej. GK Xpress) recauda en tu nombre · multi-canal · liquidación periódica al banco',
    color: 'pink',
  },
];
...
export const Paso1TipoProducto: React.FC<Paso1Props> = ({ state, setState }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {state.tipo === 'recaudadora' && (
        <div className="bg-pink-50 border border-pink-200 rounded-md p-3 text-[11px] text-pink-900">
          <strong>Nota:</strong> Una <strong>Caja Recaudadora</strong> representa a un
          tercero (ej. GK Xpress) que cobra en tu nombre y luego te liquida el saldo
          periódicamente. Soporta múltiples canales (Yape, Plin, POS, efectivo) y el
          tercero descuenta su tarifa de servicio (ej. S/ 25 por carrera). Configurá
          responsable + tarifa en Paso 2 · cuenta liquidación en Paso 3 · canales
          aceptados en Paso 4.
        </div>
      )}
    </div>
...
const PRODUCTO_HINT: Record<ProductoFinancieroNuevo, string> = {
  ...
  wise: 'Multi-moneda',
  binance: 'Crypto + fiat',
  caja_recaudadora: 'Tercero recauda · multi-canal · liquidación periódica',
};

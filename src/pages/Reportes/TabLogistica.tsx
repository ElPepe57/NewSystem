      ...
      {rankingProductos.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 bg-gray-50 border-b flex items-center justify-between">
            <h3 className="font-semibold text-gray-900">Eficiencia Logistica por Producto</h3>
            <span className="text-xs text-gray-500">{rankingProductos.length} productos con peso</span>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {rankingProductos.slice(0, 15).map((p, i) => (
                  <tr key={p.sku} className={`border-b border-gray-100 ${i < 3 ? 'bg-green-50/30' : ''} ${i >= rankingProductos.length - 3 ? 'bg-red-50/30' : ''}`}>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 bg-gray-50/50">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-3">
            ...
            <MiniStat label="Costo/lb" value={v.costoPromedioPorLibra > 0 ? `$${v.costoPromedioPorLibra.toFixed(2)}` : '—'} className={v.costoPromedioPorLibra > 0 ? 'text-indigo-600' : 'text-gray-400'} />
          </div>
              ...
              <tbody>
                {v.transferencias.slice(0, 10).map(t => (
                  <tr key={t.id} className="border-b border-gray-100">
                    ...
                    <td className="py-1.5 px-2 text-center text-gray-500">{t.pesoTotalLb ? `${t.pesoTotalLb.toFixed(1)}` : '-'}</td>

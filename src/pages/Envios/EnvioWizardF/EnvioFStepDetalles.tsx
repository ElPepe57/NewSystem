/**
 * EnvioFStepDetalles — Paso 3 del Wizard F (Transporte + Costos PEN).
 *
 * Captura el colaborador transporte (delivery/courier local), tracking y costos
 * del despacho en PEN. Estructura casi idéntica a EnvioEStepDetalles.
 */
...
import { selectUnidadesCount, selectTotalCostosPEN } from './envioWizardFTypes';

export interface EnvioFStepDetallesProps {
  state: EnvioWizardFState;
  dispatch: (action: EnvioWizardFAction) => void;
}

export const EnvioFStepDetalles: React.FC<EnvioFStepDetallesProps> = ({ state, dispatch }) => {
    ...
    <div className="space-y-5">
      <div>
        <h3 className="text-base font-semibold text-slate-900">
          Transporte y costos del despacho
        </h3>
        <p className="text-sm text-slate-600 mt-1">
          Todo en <strong>PEN</strong>. El transporte es opcional — puedes registrar un
          despacho interno sin transportista externo.
        </p>
      </div>
      ...
      <div>
        <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 block mb-2">
          Transportista / delivery <span className="text-slate-400 normal-case text-[11px]">(opcional)</span>
        </label>
        {transportistas.length === 0 ? (
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600">
            No hay transportistas locales activos. El despacho queda registrado sin
      ...
      <div>
        ...
        {state.costosPEN.length === 0 && (
          <div className="mb-2 flex flex-wrap gap-2">
            {['Delivery', 'Courier', 'Combustible', 'Peaje'].map((c) => (
              <button
                key={c}
                type="button"
                ...
                <select
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: 'EDITAR_COSTO_PEN',
                      id: c.id,
                      updates: { metodo: e.target.value as CostoPENDespacho['metodo'] },
                    })
                  }
                  disabled={!c.activo}

/**
 * EnvioT2StepCostos — Paso 4 del Wizard T2 (Costos landed).
 *
 * Captura los costos landed del envío. Aplicando D-17 y D-18:
 *   - Captura OPCIONAL (no obliga): el usuario puede saltar este paso si
 *     aún no conoce los costos. Los ingresa después en el detalle del envío.
 *   - En S44 todos los costos son `scope='envio'` implícito.
 *   - 3 presets de tarifa (Monto total · Por unidad · Variable)
 *   - Costos adicionales (fee de recepción, etc.)
 *   - Preview CTRU landed preliminar en tiempo real
 */
import React, { useEffect, useMemo } from 'react';
import { useProductoStore } from '../../../store/productoStore';
import { useTipoCambio } from '../../../hooks/useTipoCambio';
import { getEmojiPorProducto } from '../../../components/modules/ordenCompra/OCWizardV3/productoEmoji';
import { TarifaPresetSelector } from './TarifaPresetSelector';
import { CTRULandedPreview, type CTRULandedPreviewFila } from './CTRULandedPreview';
import {
...
export interface EnvioT2StepCostosProps {
  state: EnvioWizardT2State;
  dispatch: (action: EnvioWizardT2Action) => void;
}

export const EnvioT2StepCostos: React.FC<EnvioT2StepCostosProps> = ({ state, dispatch }) => {
  const productos = useProductoStore((s) => s.productos);
  const { tc } = useTipoCambio();

  // Pesos por producto (para cálculos de peso total)
  ...
  // Prellenar TC al montar si no está definido
  useEffect(() => {
  ...
  // Filas del CTRULandedPreview: una por producto en la selección
  const filasPreview: CTRULandedPreviewFila[] = useMemo(() => {
    // Agrupar unidades seleccionadas por producto
  ...
  // Fee de recepción (primer costo adicional sugerido por default)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-sky-50 border border-sky-200 rounded-xl p-3 flex items-start gap-3">
        ...
        <div className="text-xs text-sky-900">
          ...
          <div className="text-sky-800">
            El <strong>cierre operativo</strong> y el <strong>cierre financiero</strong> son
            independientes. El viajero/courier puede facturar semanas después. El CTRU queda{' '}
      ...
      {/* Inputs principales según preset */}
        ...
        <div className="divide-y divide-slate-100">
          ...
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
              <input
                type="text"
                value={feeRecepcion?.concepto ?? 'Recepción en destino'}
                onChange={(e) =>
                  dispatch({
                    type: 'EDITAR_COSTO_ADICIONAL',
                    id: 'fee-recepcion',

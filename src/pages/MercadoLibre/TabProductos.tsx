...
const MobileListingItem: React.FC<{ listing: MLProductMap }> = ({ listing }) => {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-slate-50/50">
      <span className="w-1 h-8 bg-slate-200 rounded-full shrink-0" />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium text-slate-700">S/ {listing.mlPrice?.toFixed(2)}</span>
          {listing.buyBoxStatus && listingType === 'catalogo' && (
            <BuyBoxBadge listing={listing} />
          )}
        </div>
        <p className="text-[10px] text-slate-400 mt-0.5">{listing.mlItemId}</p>
      </div>
      <a
        href={listing.mlPermalink}
        target="_blank"
        rel="noopener noreferrer"
        className="p-1 text-slate-400 hover:text-amber-600 rounded-lg shrink-0"
      >
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="flex items-start gap-3 p-3">
        {primaryListing.mlThumbnail && (
          <img src={primaryListing.mlThumbnail} alt="" className="w-14 h-14 rounded-lg object-cover bg-slate-100 shrink-0" />
        )}
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-slate-900 line-clamp-2 leading-tight">{primaryListing.mlTitle}</p>
          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
            <p className="text-[10px] text-slate-400">{primaryListing.mlItemId}</p>
            {hasMultiple && (
      ...
      <div className="grid grid-cols-3 gap-px bg-slate-100 border-t border-slate-100">
        ...
        <div className="bg-white px-3 py-2">
          <p className="text-[10px] text-slate-400 uppercase">Stock ML</p>
          <p className="text-sm font-semibold text-slate-900">{group.stockML}</p>
        </div>
        <div className="bg-white px-3 py-2">
          <p className="text-[10px] text-slate-400 uppercase">Stock ERP</p>
          {group.vinculado ? (
            <p className={`text-sm font-semibold ${stockMismatch ? 'text-orange-600' : 'text-slate-900'}`}>
      ...
      <div className="border-t border-slate-100 px-3 py-2 space-y-1.5">
        {group.mlSku && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-600 font-mono">{group.mlSku}</span>
          </div>
        )}
        {group.vinculado && group.productoNombre && (
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] text-slate-400 shrink-0">ERP</span>
            <span className="text-xs text-slate-700 font-medium truncate">{group.productoNombre}</span>
          </div>
        )}
        {!group.vinculado && (
          <button
        ...
        {primaryListing.mlListingType === 'catalogo' && primaryListing.buyBoxStatus && (
          <div className="flex items-center justify-between pt-0.5">
            <span className="text-[10px] text-slate-400">Competencia</span>
      ...
      {hasMultiple && (
        <>
          <button
            ...
            className="w-full flex items-center justify-center gap-1 py-2 border-t border-slate-100 text-xs text-slate-500 hover:bg-slate-50 transition-colors"
          >
    ...
    <tr className="bg-slate-50/50 hover:bg-slate-100/50">
      ...
      <td className="px-4 py-2">
        <div className="flex items-center gap-2 pl-4">
          <span className="w-1 h-6 bg-slate-200 rounded-full" />
      ...
      <td className="px-4 py-2 text-xs text-slate-500">{listing.mlSku || '—'}</td>
      <td className="px-4 py-2">
        {editingPrice ? (
      ...
      <td className="px-4 py-2 text-xs text-slate-500">{listing.mlAvailableQuantity}</td>
      ...
      <tr className="hover:bg-slate-50">
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-3">
            {primaryListing.mlThumbnail && (
              <img src={primaryListing.mlThumbnail} alt="" className="w-10 h-10 rounded-lg object-cover bg-slate-100" />
            )}
            <div className="min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate max-w-[200px]">{primaryListing.mlTitle}</p>
              <div className="flex items-center gap-1.5">
                <p className="text-xs text-slate-400">{primaryListing.mlItemId}</p>
                {hasMultiple && (
        ...
        <td className="px-4 py-3 text-sm text-slate-600">{group.mlSku || '—'}</td>
        ...
        <td className="px-4 py-3 text-sm">
          {group.vinculado ? (
            stockERP !== undefined && stockERP !== group.stockML ? (
              ...
              <span className="text-slate-600">{stockERP ?? '—'}</span>
            )
          ) : (
      ...
      <div className="space-y-4">
        ...
        {mlProduct && (
          <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-slate-900">{mlProduct.mlTitle}</p>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-amber-500 focus:border-amber-500"
            autoFocus
          />
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((p) => (
              ...
              >
                ...
                <LinkIcon className="w-4 h-4 text-slate-300 flex-shrink-0" />
              </button>
            ))
          )}
        </div>
      ...
      <div className="space-y-2 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-4">
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full sm:w-64 pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-amber-500 focus:border-amber-500"
            />
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onSync}
            disabled={syncing}
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 text-xs sm:text-sm bg-slate-100 hover:bg-slate-200 rounded-lg disabled:opacity-50"
          >
        ...
        <div className="space-y-4">
          ...
          {productMaps.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
              ...
              <p className="text-slate-500 mb-4">No hay productos sincronizados</p>
              <button
                onClick={onSync}
                disabled={syncing}

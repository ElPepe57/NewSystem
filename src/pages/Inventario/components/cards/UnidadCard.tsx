...
const getEstadoConfig = (estado: EstadoUnidad, pais?: string): { variant: 'success' | 'info' | 'warning' | 'default' | 'danger'; label: string; bgColor: string } => {
  ...
  switch (estado) {
    ...
    default: return { variant: 'default', label, bgColor: 'bg-slate-50' };
  }
};
...
export const UnidadCard: React.FC<UnidadCardProps> = ({
  ...
  return (
    ...
    >
      {/* Header con estado */}
      <div className={`${estado.bgColor} px-4 py-3 border-b border-slate-200`}>
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="h-10 w-10 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center flex-shrink-0">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-slate-900">{unidad.lote || '-'}</span>
          </div>
        ...
        {unidad.ordenCompraNumero && (
          <div className="mt-3 pt-3 border-t border-slate-100">
            <div className="text-xs text-slate-500">
              OC: <span className="font-medium text-slate-700">{unidad.ordenCompraNumero}</span>
            </div>
          </div>
        )}
      </div>

  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div>
        ...
        {online.length === 0 ? (
          <p className="text-sm text-slate-400 italic">Nadie más en línea</p>
        ) : (
  ...
  return (
    <div
      ...
      className={`group flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition-colors ${
        isSelf ? 'hover:bg-slate-50' : 'hover:bg-teal-50 cursor-pointer'
      }`}
    >
      ...
      <div className="relative">
        {user.photoURL ? (
          ...
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${ROLE_COLORS[user.role] || 'bg-slate-500'}`}>
      ...
      {!isSelf && (
        <MessageCircle className="h-4 w-4 text-slate-300 group-hover:text-teal-500 transition-colors flex-shrink-0" />
      )}
    </div>
  );
};

...
export const TabNavigation: React.FC<TabNavigationProps> = ({
  ...
  // Auto-scroll to active tab on mount and when active tab changes
  useEffect(() => {
      ...
      <nav ref={scrollRef} className="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto scrollbar-hide">
        {tabs.map(tab => (
          <button
            ...
            onClick={() => onTabChange(tab.id)}
            className={`
              flex items-center gap-1.5 sm:space-x-2 py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm
              transition-all duration-200 whitespace-nowrap flex-shrink-0
              ${activeTab === tab.id

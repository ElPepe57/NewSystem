    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="flex items-start gap-3 p-3">
        <input
          ...
          className="mt-1 rounded border-slate-300 text-amber-500 focus:ring-amber-500 shrink-0"
        />
        {row.mlThumbnail && (
          <img src={row.mlThumbnail} alt="" className="w-12 h-12 rounded-lg object-cover bg-slate-100 shrink-0" />
        )}
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
            ...
            <span className="text-[10px] text-slate-400">{row.productoSku || row.mlSku || row.listings[0]?.mlItemId}</span>
            {!row.vinculado && (
            ...
            {row.listings.length > 1 && (
              <span className="text-[10px] text-slate-400">{row.listings.length} pub.</span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-px bg-slate-100 border-t border-slate-100">
        <div className="bg-white px-3 py-2">
          ...
          <p className="text-sm font-semibold text-slate-900">{fmtPEN(row.mlPrice)}</p>
        </div>
        <div className="bg-white px-3 py-2">
          <p className="text-[10px] text-slate-400 uppercase">Costo CTRU</p>

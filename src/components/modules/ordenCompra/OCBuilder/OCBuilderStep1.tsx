      ...
      <div className="lg:w-[58%] flex flex-col min-h-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900">Productos del Requerimiento</h3>
            <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{state.pool.length}</span>
          </div>
          {alreadyInOCCount > 0 && (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {formatProductSubtitle(p) && (
                      <p className="text-xs text-slate-400 truncate">{formatProductSubtitle(p)}</p>
                    )}
                  ...
                  {isExpanded && (
                    <div className="px-3 py-2 space-y-1">
                      {group.productos.length === 0 ? (
                        <p className="text-xs text-slate-400 py-3 text-center">Agrega productos desde el panel izquierdo</p>
                      ) : (
                        <>
                          {group.productos.map(p => {
                            const subtitle = formatProductSubtitle(p);
                            return (
                              <div key={p.productoId} className="flex items-center gap-2 py-1.5 text-sm border-b border-slate-50 last:border-0">
                                <div className="flex-1 min-w-0">
                                  <p className="text-slate-800 truncate text-xs">{p.marca} - {p.nombreComercial}</p>
                                  {subtitle && <p className="text-[10px] text-slate-400 truncate">{subtitle}</p>}
                                </div>
                                <span className="text-xs text-slate-500 flex-shrink-0">{p.cantidad} ud</span>
                                <span className="text-xs font-medium text-slate-700 flex-shrink-0 w-16 text-right">

  ...
  return (
    <div ref={ref} className="relative mx-3 mt-3">
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800/40 hover:bg-slate-800/70 border border-slate-700/30 hover:border-slate-600/50 w-full transition-all duration-150"
      >
        <span
        ...
        <ChevronDown className={`h-4 w-4 text-slate-400 transition-transform duration-200 flex-shrink-0 ${open ? 'rotate-180' : ''}`} />
      </button>

      {/* Dropdown */}
      {open && (
        <div className="absolute left-0 right-0 mt-1 bg-slate-800 rounded-lg shadow-xl border border-slate-700/50 z-50 py-1 max-h-64 overflow-y-auto">
          {/* Opción "Todas" */}
          <button

...
export const LineaNegocioForm: React.FC<LineaNegocioFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* Nombre */}
      <div>
        <label htmlFor="ln-nombre" className="block text-sm font-medium text-slate-700 mb-1">
      ...
      {/* Codigo */}
      <div>
        <label htmlFor="ln-codigo" className="block text-sm font-medium text-slate-700 mb-1">
        ...
        <input
          ...
          placeholder="SUP"
          maxLength={5}
          className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 uppercase ${
            errors.codigo ? 'border-red-300' : 'border-slate-300'
          }`}
        />
        {errors.codigo && <p className="mt-1 text-sm text-red-600">{errors.codigo}</p>}
        <p className="mt-1 text-xs text-slate-400">{codigo.length}/5 caracteres</p>
      </div>

      {/* Descripcion */}
      <div>
        <label htmlFor="ln-descripcion" className="block text-sm font-medium text-slate-700 mb-1">
          Descripcion
        </label>
        <textarea
          id="ln-descripcion"
      ...
      <div>
          ...
          {EMOJI_OPTIONS.map((emoji) => (
            <button
              ...
              className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center transition-all border ${
                icono === emoji
                  ? 'border-slate-800 bg-slate-100 ring-2 ring-offset-1 ring-slate-800 scale-110'
        ...
        <input
          ...
          maxLength={4}
          className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 text-center text-lg"
        />
      </div>
      ...
      <div className="flex items-center justify-between py-2">
        <div>
          <label htmlFor="ln-activa" className="text-sm font-medium text-slate-700">
            Activa
          </label>
          <p className="text-xs text-slate-400">Las lineas inactivas no aparecen en seleccion</p>
        </div>
        <button
          id="ln-activa"
          type="button"
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            activa ? 'bg-teal-600' : 'bg-slate-300'
          }`}
        >
          <span
      ...
      <div className="border border-slate-200 rounded-lg p-4 bg-slate-50">
        <p className="text-xs text-slate-500 mb-2 font-medium">Vista previa</p>
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-semibold text-slate-900">{nombre || 'Nombre de la linea'}</p>

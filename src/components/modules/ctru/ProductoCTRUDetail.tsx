    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 pb-3 border-b">
          ...
          <span className="text-sm text-slate-500">{p.unidadesActivas} activas / {p.totalUnidades} total</span>
          {p.ventasCount > 0 && (
            <span className="text-sm text-slate-500">| {p.ventasCount} ventas</span>
          )}
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {visibleLayers.map((layer) => (
            ...
            >
              <div className="flex flex-col">
                ...
                {'note' in layer && layer.note && <span className="text-[10px] text-slate-400">{layer.note}</span>}
              </div>
              <div className="flex items-center gap-3">
                {layer.usd && (
                  <>
                    <span className="text-xs text-slate-500">{layer.usd}</span>
                    <ArrowRight className="w-3 h-3 text-slate-400" />
                  </>
                )}
                <span className="text-sm font-semibold text-slate-800 w-24 text-right">{layer.pen}</span>
                <span className="text-xs text-slate-500 w-14 text-right">({layer.pct})</span>
              </div>
            </div>
          ))}
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
            <span className="text-sm font-medium text-slate-700">= Costo Inventario (Capas 1-5)</span>
            <span className="text-base font-bold text-slate-700">{formatCurrency(p.costoInventarioProm)}</span>
          </div>
          ...
          {/* Costo Total Real */}
          <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg">
            <span className="text-sm font-bold text-slate-800">= Costo Total Real</span>
            <span className="text-lg font-bold text-slate-900">{formatCurrency(costoTotal)}</span>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="text-xs font-medium text-slate-600 uppercase tracking-wide">Precios sugeridos por margen</div>
          ...
          {p.pricing.precioActual > 0 && (
            <>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg">
                <span className="text-sm font-medium text-slate-700">Precio actual prom.</span>
                <span className="text-base font-bold text-slate-900">{formatCurrency(p.pricing.precioActual)}</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          ...
          <div className="bg-slate-50 p-3 rounded-lg text-center">
            ...
            <div className="text-lg font-bold text-slate-800">{formatUSD(p.costoCompraUSDProm)}</div>
          </div>
        </div>
      </div>
    </div>

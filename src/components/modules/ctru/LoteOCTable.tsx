        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr className="border-b border-slate-200">
              ...
              <th className="text-left py-3 px-2 font-medium text-slate-600">Proveedor</th>
              <th className="text-left py-3 px-2 font-medium text-slate-600">Fecha</th>
              <th className="text-right py-3 px-2 font-medium text-slate-600">TC Compra</th>
              <th className="text-right py-3 px-2 font-medium text-slate-600">TC Pago</th>
              <th className="text-right py-3 px-2 font-medium text-slate-600">Uds.</th>
              <th className="text-right py-3 px-2 font-medium text-slate-600">CTRU Prom.</th>
              <th className="text-right py-3 px-2 font-medium text-slate-600">Composicion</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((lote) => {
              ...
              return (
                <React.Fragment key={lote.ordenCompraId}>
                  ...
                  >
                    ...
                    <td className="py-3 px-2 font-medium text-slate-900">{lote.proveedorNombre}</td>
                    <td className="py-3 px-2 text-slate-600">{formatDate(lote.fechaRecepcion)}</td>
                    <td className="py-3 px-2 text-right text-slate-600">{formatTC(lote.tcCompra)}</td>
                    <td className="py-3 px-2 text-right">
                      <span className={tcDiff > 0 ? 'text-red-600' : tcDiff < 0 ? 'text-green-600' : 'text-slate-600'}>
                      ...
                      <td colSpan={9} className="bg-slate-50 p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
                          ...
                          <div className="bg-white p-3 rounded-lg border border-l-4 border-l-gray-400">
                            ...
                            <div className={`text-sm font-semibold ${tcDiff > 0 ? 'text-red-600' : tcDiff < 0 ? 'text-green-600' : 'text-slate-600'}`}>
                              {tcDiff !== 0
                        ...
                        <table className="w-full text-xs">
                          <thead className="bg-slate-50">
                            <tr className="border-b">
                              ...
                              <th className="text-right py-2 px-2 text-slate-500">Cant.</th>
                              <th className="text-right py-2 px-2 text-slate-500">Costo USD</th>
                              <th className="text-right py-2 px-2 text-slate-500">CTRU Prom.</th>
                            </tr>
                          </thead>
                          <tbody>

...
export const ProductoSearchCotizaciones: React.FC<ProductoSearchCotizacionesProps> = ({
  ...
  const getDisponibilidadStyle = (fuente: string) => {
    switch (fuente) {
      ...
      case 'usa_viajero':
      case 'origen_viajero':
        return { bg: 'bg-sky-100', text: 'text-sky-700', icon: Plane };
      case 'usa_almacen':
      case 'origen_almacen':
        return { bg: 'bg-teal-100', text: 'text-teal-700', icon: Plane };
      case 'virtual':
      default:
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
          ...
          <button
            ...
            className={`px-3 py-2 border rounded-md shadow-sm transition-colors flex-shrink-0 ${
              showScanner
                ...
                : 'bg-white border-slate-300 text-slate-500 hover:text-teal-600 hover:border-teal-300 active:bg-slate-50'
            }`}
          >
      ...
      {showScanner && !disabled && !value && (
        <div className="mt-2 p-3 sm:p-4 bg-white border border-slate-200 rounded-lg shadow-lg z-50 relative">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-500">Escanear codigo de barras</span>
            <button
              type="button"
              onClick={() => setShowScanner(false)}
              className="p-1 text-slate-400 hover:text-slate-600 active:bg-slate-100 rounded"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 mt-1 sm:mt-1.5 flex-wrap">
                      ...
                      <div className="flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-emerald-50 rounded text-[10px] sm:text-xs">
                        ...
                        <span className="font-bold text-emerald-700">{disp.peru}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-sky-50 rounded text-[10px] sm:text-xs">
                        ...
                        <span className="font-bold text-sky-700">{disp.usa}</span>
                      </div>
                      ...
                      {inv && (
                        <div className="flex items-center gap-0.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-slate-100 rounded text-[10px] sm:text-xs">
                          ...
                          <span className="font-semibold text-slate-700">S/{inv.precioPERUPromedio.toFixed(0)}</span>
                        </div>
                      )}
      ...
      {value && (
        <div className="mt-2 p-2 sm:p-3 bg-teal-50 border border-teal-200 rounded-lg">
          <div className="flex items-center justify-between gap-1 mb-2">
            <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
              <Info className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-teal-600 flex-shrink-0" />
              <span className="text-xs sm:text-sm font-medium text-teal-800 truncate">
                Info para cotizar
              </span>
            </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5 sm:gap-2">
            ...
            <div className="bg-white/60 rounded p-1.5 sm:p-2 text-center">
              ...
              <p className="font-bold text-sm sm:text-lg text-teal-600">{value.tiempoEntregaEstimado}d</p>
            </div>
            ...
            {/* Precio entrada */}
            <div className="bg-white/60 rounded p-1.5 sm:p-2 text-center">
              <p className="text-[10px] sm:text-xs text-slate-600">Entrada</p>
          ...
          {value.precioPERUMin && value.precioPERUMax && (
            <div className="mt-2 pt-2 border-t border-teal-200">
              <div className="flex items-center justify-between text-[10px] sm:text-xs">
                <span className="text-slate-600">Rango mercado:</span>

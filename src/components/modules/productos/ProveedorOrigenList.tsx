...
interface ProveedorOrigenListProps {
  proveedores: ProveedorUSAFormData[];
  onChange: (proveedores: ProveedorUSAFormData[]) => void;
  disabled?: boolean;
  lineaNegocioId?: string;
  /** País de origen del producto: 'USA', 'China', 'Corea', etc. */
  paisOrigen?: string;
  /** Sugerencias adicionales de proveedores (legacy) */
  sugerenciasProveedores?: string[];
}

export const ProveedorOrigenList: React.FC<ProveedorOrigenListProps> = ({
  proveedores,
  onChange,
  disabled = false,
  lineaNegocioId,
  ...
  const paisConfig = paisOrigen ? PAISES_CONFIG[paisOrigen] : null;
  const nombrePais = paisConfig?.nombre ?? paisOrigen ?? '';

  const labelProveedor = nombrePais ? `Proveedores ${nombrePais}` : 'Proveedores';
  const labelAgregar = nombrePais ? `Agregar Proveedor ${nombrePais}` : 'Agregar Proveedor';
  const labelNuevo = nombrePais ? `Nuevo Proveedor ${nombrePais}` : 'Nuevo Proveedor';
  const paisDefault = paisOrigen ?? '';
  ...
  // Filtrar proveedores según país de origen
  const proveedoresDisponibles = useMemo(() => {
    if (paisOrigen) return proveedoresActivos.filter(p => p.pais === paisOrigen);
    return proveedoresActivos;
  }, [proveedoresActivos, paisOrigen]);
...
const ProveedorExpandido: React.FC<ProveedorExpandidoProps> = ({
  ...
  const [nuevoProveedor, setNuevoProveedor] = useState<Partial<ProveedorFormData>>({
    nombre: '',
    tipo: 'distribuidor',
    // Mantener USA como fallback final para compatibilidad con docs existentes
  ...
  return (
    <div className="px-3 pb-3 pt-0 border-t border-slate-100">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
        ...
        {/* Impuesto/Tax: aplica a USA (sales tax) y puede aplicar a otros países con VAT */}
        <div className="flex items-end gap-2">
          <Input
            ...
            placeholder="ej: 8.25"
            helperText="Sales tax / VAT según país"
            disabled={disabled}
          />
          <div className="pb-2">

...
/**
 * F14.6 · decisión D2 · Atribuye el SALDO INICIAL de una cuenta a un socio como
 * aporte fundacional, SIN doble conteo.
 *
 * Clave anti-doble-conteo: NO crea movimiento ni toca el saldo de la cuenta
 * (ese saldo ya lo seteó `crearProductoFinanciero` desde `saldoInicial`). Solo
 * escribe el espejo en `aportesCapital` (marcado `esFundacional`), que es de
 * donde `getTotalAportesCapital` → `cashAportadoPEN` lee. Así el capital queda
 * atribuido al socio una sola vez (origen), reflejado en el activo caja (saldo).
 *
 * Ejemplo: cuenta BCP con saldoInicial S/52,000 declarada "aporte de José" →
 * el saldo de BCP es S/52,000 (activo) y José muestra S/52,000 de cash aportado
 * (origen). Antes este capital quedaba "sin dueño".
 */
export async function registrarAporteFundacional(
  data: {
    socioId: string;
    socioNombre: string;
    monto: number;
  ...
  if (data.monto <= 0) throw new Error('El monto del aporte fundacional debe ser mayor a 0');
  if (!data.socioId) throw new Error('Falta el socio del aporte fundacional');

  const montoEquivalentePEN =
    data.moneda === 'USD' ? data.monto * data.tipoCambio : data.monto;
  ...
  const doc: Record<string, unknown> = {
    ...
    tipoCambio: data.tipoCambio,
    fecha: Timestamp.now(),
    esFundacional: true, // marca · saldo de apertura atribuido (no es movimiento)
    movimientoId: null,
    creadoPor: userId,
    fechaCreacion: Timestamp.now(),
  };
  if (data.cuentaId) doc.cuentaOrigenId = data.cuentaId;
  if (data.cuentaNombre) doc.concepto = `Saldo inicial · ${data.cuentaNombre}`;

  const ref = await addDoc(collection(db, COLLECTIONS.APORTES_CAPITAL), doc);
  logger.success(
    `Aporte fundacional · ${data.monto} ${data.moneda} atribuido a ${data.socioNombre} (sin afectar saldo)`,
  );
  return ref.id;
}

/**
 * Utilidad para migrar productos desde el CSV del sistema antiguo
 * Ejecutar desde la consola del navegador o importar en un componente
 */
...
// Datos del CSV del sistema antiguo
...
/**
 * Ejecuta la migración de productos
 * @param userId - ID del usuario que realiza la migración
...
export async function migrarProductos(
  ...
  onProgress?.(`Encontrados ${clavesExistentes.size} productos existentes`, 5);

  // Rastrear duplicados en el CSV
  ...
  for (let i = 0; i < productosCSV.length; i++) {
    ...
    if (clavesExistentes.has(clave) || clavesYaProcesadas.has(clave)) {
      result.omitidos++;
      result.productosOmitidos.push(`${productoCSV.marca} - ${productoCSV.nombreComercial} (${productoCSV.contenido})`);
      continue;
    }
    ...
    try {
      ...
      const productoCreado = await ProductoService.create(formData, userId);
      result.creados++;
      result.productosCreados.push(`${productoCreado.sku}: ${productoCSV.marca} - ${productoCSV.nombreComercial}`);

      // Pequeña pausa cada 10 productos
      if (result.creados % 10 === 0) {
        await new Promise(resolve => setTimeout(resolve, 100));
      }

    } catch (error: any) {
      result.errores++;
      result.erroresDetalle.push(`${productoCSV.marca} - ${productoCSV.nombreComercial}: ${error.message}`);
    }
  }
...
// Exportar los datos del CSV para uso externo
export { productosCSV };
